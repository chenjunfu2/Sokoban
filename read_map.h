...
//打开地图
static inline bool open_map(fstream& fin_b, map_a& mpa)
{
...
//关闭地图
static inline void close_map_fin(fstream& fin_b)
{
	fin_b.clear();
	fin_b.close();
}
...
//读取关卡号
static inline bool read_checkpoint(fstream& fin_b, map_a& mpa, map_j& mpj, int mpn)
{
...
conti:
	...
	{
		if (fin_b.eof())
			no_good(mpa.error, fst_eof);//-1 EOF
		else
