...
#define OPB(B,N) ((B)|=((decltype(B))(1)<<(N-1))
#define CLB(B,N) ((B)&=(~((decltype(B))(1)<<(N-1))))
#define ISB(B,N) ((bool)(((B)>>(N-1))&(decltype(B)(1))))
#define MAPBIN ((ULL)(STU('M', 1) | STU('A', 2) | STU('P', 3) | STU(' ', 4) | STU('B', 5) | STU('I', 6) | STU('N', 7) | STU('\0', 8)))
#define FLAG(F8,F7,F6,F5,F4,F3,F2,F1) ((BYT)(BTC(F8, 8) | BTC(F7, 7) | BTC(F6, 6) | BTC(F5, 5) | BTC(F4, 4) | BTC(F3, 3) | BTC(F2, 2) | BTC(F1, 1)))
...
struct BinFileHead
{
	...
	size_t size = 0;	//文件大小标记sizeof(BinFileHead)
